	...
	<table class="table ulisting-table">
		...
		?>
			<tr>
				...
				<td>
					<?php if($model->payment_type == \uListing\Lib\PricingPlan\Classes\StmPricingPlans::PRICING_PLANS_PAYMENT_TYPE_SUBSCRIPTION): ?>
						<?php echo stm_convert_date_format($model->expired_date)?>
					<?php else:?>
						--/--/----
					<?php endif;?>
				</td>
				<td><?php echo stm_convert_date_format($model->updated_date).' '.stm_convert_time_format($model->updated_date)?></td>
				<td>
...
<?php else:?>
	<div class="stm-row stm-justify-content-center p-t-30 stm-empty-msg">
        ...
        <div class="stm-col-12">
			<a class="btn btn-success" href="<?php echo stml_get_link('pricing_plan')?>"><?php _e('Buy plan', 'motors')?> </a>
		</div>
	</div>
<?php endif;?>
