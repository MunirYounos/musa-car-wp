...
<div <?php echo \uListing\Classes\Builder\UListingBuilder::generation_html_attribute($element) ?>>
    ...
    foreach ($items as $val) {
        ...
        $gallery_items[] = [
            'sort' => $val->sort,
            'full' => ($full) ? $full : [get_placeholder_image_url()],
            'thumbnail' => ($thumbnail) ? $thumbnail : [get_placeholder_image_url()],
        ];
    }
...
<div class="stm-gallery-custom-style">
    <?php echo \uListing\Classes\StmListingTemplate::load_template('builder/attribute/' . $style, array('args' => $args, 'element' => $element)); ?>
</div>
<?php endif; ?>
